...
pub(crate) fn verify_last_pass<S: EventSink>(
	...
	let mut file = File::open(path).change_context(Error::SystemProblem(FSProblem::Opening, format!("{}",path.to_string_lossy())))?;
	file.sync_all().change_context(Error::SystemProblem(FSProblem::Read, format!("{}",path.to_string_lossy())))?;
	file.seek(SeekFrom::Start(0)).change_context(Error::SystemProblem(FSProblem::Read, format!("{}",path.to_string_lossy())))?;
