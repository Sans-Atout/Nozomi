...
///
/// ## Argument :
/// * `path` (&PathBuf) : path that you want to erase using DOD 522 022 ME overwrite method
///
/// ## Return
/// * `()`
#[cfg(not(feature = "error-stack"))]
pub(crate) fn overwrite_file(path: &PathBuf) -> Result<()> {
    ...
    for pass in 0..FIXED_PATTERNS.len() {
        ...
        file.seek(SeekFrom::Start(0)).map_err(|_| Error::OverwriteError(Method::Dod522022ME, pass as u32))?;
        ...
        while remaining > 0 {
            ...
            file.write_all(&buffer[..write_size]).map_err(|_| Error::OverwriteError(Method::Dod522022ME, pass as u32))?;
            remaining -= write_size as u64;
        }

        file.flush().map_err(|_| Error::OverwriteError(Method::Dod522022ME, pass as u32))?;
    }
...
pub(crate) fn overwrite_file(path: &PathBuf) -> Result<()> {
    ...
    for pass in 0..FIXED_PATTERNS.len() {
        // rewind start of file
        file.seek(SeekFrom::Start(0)).change_context(Error::OverwriteError(Method::Dod522022ME, pass as u32))?;
        ...
        while remaining > 0 {
            ...
            file.write_all(&buffer[..write_size]).change_context( Error::OverwriteError(Method::Dod522022ME, pass as u32))?;
            remaining -= write_size as u64;
        }

        file.flush().change_context(Error::OverwriteError(Method::Dod522022ME, pass as u32))?;
    }
